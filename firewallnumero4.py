...
class RestFirewallAPI(app_manager.RyuApp):
    ...
    def packet_in_handler(self, ev):
        ...
        print(' ')
        print('id del sw destino',dpid_str)
        print('mac destino',dst,' y mac origen:', src)
        ...
        if eth.ethertype == ether_types.ETH_TYPE_IP:
            ...
            print(" ip source es: "+srcip)
            print(" ip destino es: "+dstip)

            ipdst = dstip+'/32'
            ipdst = str(ipdst)
            nuevaipdst = dstip.replace(".","")
            print('la ip que uiero tomar de destino', nuevaipdst)
            nuevaipdst = str(nuevaipdst)
            valor = nuevaipdst[-1:]
            print(valor)
            ipsrc = srcip+'/32'
            ipsrc = str(ipsrc)
            ...
            if protocol == in_proto.IPPROTO_ICMP:
                ...
                prioridad = '1'
                flujo1 = {REST_SRC_IP: ipsrc, REST_DST_IP: ipdst, REST_NW_PROTO: REST_NW_PROTO_ICMP, REST_ACTION:  REST_ACTION_ALLOW, REST_PRIORITY :prioridad}
                flujo2 = {REST_SRC_IP: ipdst, REST_DST_IP: ipsrc, REST_NW_PROTO: REST_NW_PROTO_ICMP, REST_ACTION:  REST_ACTION_ALLOW, REST_PRIORITY :prioridad}

                sw123 = str(dpid_str)
                data22 = str(flujo1)
                data = data22.replace("'",'"')
                url =  'http://localhost:8080/firewall/rules/'+sw123
                print(url)
                #destino
                ...
                data33 = str(flujo1)
                data = data33.replace("'",'"')
                url =  'http://localhost:8080/firewall/rules/000000000000000'+valor
                print(url)
                #destino
                req = urllib2.Request(url, data, {'Content-Type': 'application/json'})
                f = urllib2.urlopen(req)
                ...
                self.logger.info(t.src_port)
                self.logger.info(t.dst_port)
                puerto1 = t.src_port
                puerto2 = t.dst_port
                ...
                query = "SELECT bw FROM mydatabase.default_values WHERE protocol='" + \
                    variable+"' AND dstport='"+p2+"'"
                print(query)
                ...
                print(myresult)
                print('EL RESULTADO DE LA BASE DE DATOS ---------------------------------------------------------------------------------------------------')
                print(' el ancho de banda es :' ,myresult[0])
                print(' accion a realizar: ',myresult[1])
                print(' la cola a la que se le asigna ',myresult[2])

                set_bw = myresult[0]
                set_action = myresult[1]
                set_queue = myresult[2]

                set_bw = str(set_bw)
                set_action = str(set_action)
                set_queue = str(set_queue)

                prioridad = '1'

                flujo3 = {REST_SRC_IP: ipsrc, REST_DST_IP: ipdst, REST_NW_PROTO: REST_NW_PROTO_UDP, REST_ACTION: set_action, REST_PRIORITY :prioridad, REST_TP_DST: '9001'}
                print('ESTE ES EL FLUJO 5 ',flujo3)
                ...
                if myresult:
                    ...
                    print('-----------------------------------------------------------------------------------------------------------------------')
                    queue_flow2 = {"match":{"nw_dst":ipdst,"nw_src":ipsrc,"nw_proto":REST_NW_PROTO_TCP,"tp_dst":p2},"actions":{"queue":set_queue}}
                    print('    ')
                    print(queue_flow2)
                    sw1234 = str(dpid_str)
                    data22 = str(queue_flow)
                    data = data22.replace("'",'"')
                    ...
                    url =  'http://localhost:8080/qos/rules/'+sw1234
                    print(url)
                    #destino
                    req = urllib2.Request(url, data, {'Content-Type': 'application/json'})
                    f = urllib2.urlopen(req)
                ...
                self.logger.info(u.src_port)
                self.logger.info(u.dst_port)
                puerto1 = u.src_port
                puerto2 = u.dst_port
                x = REST_NW_PROTO_UDP
                variable = 'UDP'
                ...
                query = "SELECT bw, action, queue FROM mydatabase.default_values WHERE protocol='" + \
                    variable+"' AND dstport='"+p2+"'"
                print(query)
                ...
                flujo5 = {REST_SRC_IP: ipsrc, REST_DST_IP: ipdst, REST_NW_PROTO: REST_NW_PROTO_UDP, REST_ACTION: set_action, REST_PRIORITY :prioridad, REST_TP_DST: p2}
                print('ESTE ES EL FLUJO 5 ',flujo5)

                print(set_bw)
                print(set_action)
                print(set_queue)
                
                print(set_action == 'DENY')
                print(set_action == str('DENY'))
                ...
                if set_action == 'DENY':
                    ...
                    print('-----------------------------------------------------------------------------------------------------------------------')
                    queue_flow = {"match":{"nw_dst":ipdst,"nw_src":ipsrc,"nw_proto":REST_NW_PROTO_UDP,"tp_dst":p2},"actions":{"queue":set_queue}}
                    print('    ')
                    print(queue_flow)
                    sw1234 = str(dpid_str)
                    data22 = str(queue_flow)
